...
class WebsiteSale(odoo.addons.website_sale.controllers.main.WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', brand=None,ppg=False, **post):
        ...
        if brand:
            ...
            brand_id = product_designer_obj.search([('id', '=', int(brand))]).id
    ...
    def product_brands(self, **post):
        ...
        for brand_id in brand_ids:
            liste.append(brand_obj.browse(int(brand_id)))

        # for brand_rec in brand_obj.browse([1,2]):
        #     brand_values.append(brand_rec)

        keep = QueryURL('/page/product_brands', brand_id=[])
        values = {'brand_rec': liste, 'keep': keep}
        if post.get('search'):
